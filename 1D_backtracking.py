...
if Test_run:
    ...
    output_path = '/storage/shared/oceanparcels/output_data/' + \
        f'data_Claudio/set_19_1D/set19_1D_{frag_timescale}.zarr'
    chunking_express = 8
...
# %%Looking for the index of the lon lat cell where the sampling locations is in the model
www = xr.open_dataset(wfiles[0])
wlats = www['nav_lat'].values
wlons = www['nav_lon'].values
lon_rest = abs(wlons[0,:] - lon_sample)
lat_rest = abs(wlats[:,0] - lat_sample)
lon_index = np.where(lon_rest == lon_rest.min())[0][0]
lat_index = np.where(lat_rest == lat_rest.min())[0][0]

margins = 5
indices = {'lat': range(lat_index-margins, lat_index+margins), 
           'lon': range(lon_index-margins, lon_index+margins)}
