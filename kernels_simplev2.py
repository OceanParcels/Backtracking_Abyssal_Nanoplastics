...
def Fragmentation(particle, fieldset, time):
    """
    Kernel for de-fragmentation of particles with five size classes.
    ...
    """
    N_total = 170.5 # total number of particles in fragmentation event
    ...
    if particle.radius < 1e-4:
        ...
        if ParcelsRandom.random(0., 1.) > fragmentation_prob:
            ...
            plim4 = 128/N_total
            plim3 = plim4 +32/N_total
            plim2 = plim3 + 8/N_total 
            plim1 = plim2 + 2/N_total
            # plim0 = plim1 + 0.5/N_total
            
            if nummer <= plim4:
                particle.radius = 16*particle.radius
                
            elif (nummer > plim4) and (nummer <= plim3):
                particle.radius = 8*particle.radius
            
            elif (nummer > plim3) and (nummer <= plim2):
